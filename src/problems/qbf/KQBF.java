...
public class KQBF implements Evaluator<Integer> {
	...
	public void setVariables(Solution<Integer> sol) {
		...
		resetVariables();
		if (!sol.isEmpty()) {	
			// See if it's necessary to save weights that will be used on the final calculation
	...
	public static void main(String[] args) throws IOException {
		...
		for (int i = 0; i < 10000000; i++) {
			...
			Double eval = KQBF.evaluateQBF(new Solution<>());
		...
		System.out.println("f(x) = " + KQBF.evaluateQBF(new Solution<>()));
